    ...
    <section className="py-16 lg:py-24 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <p className="text-gold font-medium tracking-widest uppercase mb-3">{t("ongoing.subtitle")}</p>
          <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-4">{t("ongoing.title")}</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">{t("ongoing.desc")}</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-6xl mx-auto">
          {ongoingProjects.map((project, index) => (
            <div key={index} className="group relative aspect-[4/3] rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              <img src={project.src} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              ...
              <div className="absolute top-3 right-3">
                <span className="bg-gold/90 text-primary text-xs font-bold px-2 py-1 rounded-full">{t("ongoing.badge")}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center mt-10">
          <Link to="/projects" className="inline-flex items-center gap-2 bg-gold hover:bg-gold-light text-primary font-semibold px-6 py-3 rounded-lg transition-colors">
            {t("ongoing.view_all")}
